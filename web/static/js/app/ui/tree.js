...
import { NODE_R } from "../core/config.js";
...
/**
    * Reset the stable layout cache so the next render recalculates
*/
...
/**
    * Render the radial or forest-mode visual tree into an SVG element
    *
    * @param {string}   selector        CSS selector for the SVG element
    * @param {object[]} nodes           Tree data
    * @param {object}   expandedPaths   Map of path -> expanded
    * @param {function} onToggle        Called when a dir node is clicked
    * @param {string}   toolMode        "pan" | "select"
    * @param {object}   selectedPaths   Map of path -> selected
    * @param {object[]} groups          Group definitions
    * @param {function} onToggleSelect  Called with path when selection toggled
    * @param {string}   viewMode        "1" (tree) | "2" (forest)
*/
export function renderVisualTree(
    selector,
    nodes,
    expandedPaths,
    onToggle,
    ...
    var root = filterExpandedTree(nodes, expandedPaths);
    if (!root) {
        return;
    }
    ...
    var layout = computeRadialLayout(root, svgW, svgH);
    if (!layout) {
        return;
    }

    var selectMode = toolMode === "select";
    var pathColor = buildGroupPathColorMap(groups);

    var svg = d3.select(selector)
        .attr("width", svgW)
        .attr("height", svgH);

    installResizeHandler(svgEl, svgW, svgH, function () {
    ...
    drawRadialLinks(g, root);

    var node = drawRadialNodes(g, root, pathColor, layout.visibleLabels, selectedPaths);

    wireNodeInteractions(svg, node, parent, selectMode, onToggle, onToggleSelect);

    initZoom(svg, g, root, svgW, svgH, selectMode);
}
...
/**
    * Save/restore zoom state when switching between tree and forest mode
*/
function switchZoomState(viewMode) {
...
/**
    * Build a d3 hierarchy from tree data, keeping only expanded branches
    *
    * @param {object[]} nodes         Raw tree data
    * @param {object}   expandedPaths Map of path -> expanded
    * @returns {object|null} d3 hierarchy root or null
*/
function filterExpandedTree(nodes, expandedPaths) {
    function filter(list) {
        ...
        for (var i = 0; i < list.length; i++) {
            ...
            if (n.children && n.children.length) {
                children = expandedPaths[n.path] ? filter(n.children) : [];
            }
...
/**
    * Position root children in wedges, set x/y on all nodes
    *
    * @param {object} root d3 hierarchy root
    * @param {number} svgW SVG width in px
    * @param {number} svgH SVG height in px
    * @returns {{depthStep:number, visibleLabels:object}|null}
*/
function computeRadialLayout(root, svgW, svgH) {
    var maxDepth = 0;
    ...
    return {
        depthStep: depthStep,
        visibleLabels: computeVisibleLabels(root, depthStep),
    };
}


/**
    * Build a path -> color lookup from all groups
    *
    * @param {object[]} groups Group definitions
    * @returns {object} Map of path -> color
*/
function buildGroupPathColorMap(groups) {
    var pathColor = {};
...
/**
    * Install or replace the window resize handler that re-renders the tree
    *
    * @param {Element}  svgEl    SVG DOM element
    * @param {number}   svgW     Current SVG width
    * @param {number}   svgH     Current SVG height
    * @param {function} rerender Callback to re-render
*/
function installResizeHandler(svgEl, svgW, svgH, rerender) {
    if (_resizeHandler) {
...
/**
    * Show a placeholder text when no groups exist in forest mode
*/
function showEmptyForest(svg, svgW, svgH) {
    svg.append("text")
        .attr("x", svgW / 2)
        .attr("y", svgH / 2)
...
/**
    * Draw curved links between parent and child nodes
    *
    * @param {object} g    d3 selection of the root <g>
    * @param {object} root d3 hierarchy root
*/
function drawRadialLinks(g, root) {
...
/**
    * Draw circles, labels and tooltips for every visible node
    *
    * @param {object} g              d3 selection of the root <g>
    * @param {object} root           d3 hierarchy root
    * @param {object} pathColor      Map of path -> color
    * @param {object} visibleLabels  Map of path -> visible
    * @param {object} selectedPaths  Map of path -> selected
    * @returns {object} d3 selection of node groups (for event wiring)
*/
function drawRadialNodes(g, root, pathColor, visibleLabels, selectedPaths) {
    ...
    circle.attr("r", function (d) { return NODE_R[d.data.type] || 7; });
    ...
    node.append("title")
        .text(function (d) { return d.data.path || d.data.name; });
...
/**
    * Wire click handlers for select mode or pan/toggle mode
    *
    * @param {object}   svg             d3 selection of the SVG element
    * @param {object}   node            d3 selection of node groups
    * @param {Element}  parent          Parent element
    * @param {boolean}  selectMode      True when in selection mode
    * @param {function} onToggle        Called when a dir is clicked (pan mode)
    * @param {function} onToggleSelect  Called with path when selection toggled
*/
function wireNodeInteractions(svg, node, parent, selectMode, onToggle, onToggleSelect) {
    if (selectMode) {
...
/**
    * Initialise zoom behaviour: fit-to-screen or restore saved state
    *
    * @param {object}  svg        d3 selection of the SVG element
    * @param {object}  g          d3 selection of the root <g>
    * @param {object}  root       d3 hierarchy root
    ...
    * @param {boolean} selectMode True when in selection mode
*/
function initZoom(svg, g, root, svgW, svgH, selectMode) {
    var initT;
    ...
    if (_zoomT) {
        ...
        initT = computeFitTransform(root, svgW, svgH);
    ...
    var zoom = d3.zoom()
        .scaleExtent([0.1, 5])
        .filter(function () { return !selectMode; })
...
/**
    * Project a d3 hierarchy node to polar coordinates
    *
    * @param {object} d d3 hierarchy node with x, y
...
/**
    * Compute a zoom transform that fits the entire tree in the viewport
    *
    * @param {object} root d3 hierarchy root
    * @param {number} svgW SVG width in px
    * @param {number} svgH SVG height in px
    * @returns {object} d3.zoomIdentity with translate/scale applied
*/
function computeFitTransform(root, svgW, svgH) {
...
/**
    * Compute a stable weight-and-wedge layout for the root level
    *
    * @param {object[]} nodes Tree data (mutated in-place)
*/
...
/**
    * Recursively position child nodes in their parent wedge
    *
    * @param {object[]} wNodes     Array of d3 hierarchy nodes
    * @param {number}   startAngle Start angle in radians
    * @param {number}   endAngle   End angle in radians
    * @param {number}   depth      Current depth level
    * @param {number}   dStep      Radial step between depths
*/
    ...
    if (r > 0 && adjusted.length > 1) {
        for (var pass = 0; pass < 5; pass++) {
            ...
            for (var i = 1; i < adjusted.length; i++) {
                var cr = NODE_R[adjusted[i].node.data.type] || 7;
                var pr = NODE_R[adjusted[i - 1].node.data.type] || 7;
...
/**
    * Grow radius until all labels and circles fit without overlap
    *
    * @param {object} root     d3 hierarchy root
    * @param {number} maxDepth Maximum depth in the tree
    * @param {number} svgW     SVG width in px
    * @param {number} svgH     SVG height in px
    * @returns {number} Fitted radius
*/
...
/**
    * Determine which node labels fit in the available circumference
    *
    * @param {object} root      d3 hierarchy root
    * @param {number} depthStep Radial distance between depth levels
    * @returns {object} Map of path -> visible
*/
...
/**
    * Build a path -> type lookup from the tree data
    *
    * @param {object[]} nodes Tree nodes
    * @returns {object} Map of path -> type
*/
...
/**
    * Build a minimal tree containing only the paths in a group
    *
    * @param {string[]} paths      Group paths
    * @param {object}   typeLookup Map of path -> type
    * @returns {object} Virtual root node
*/
...
/**
    * Render the forest (group orbit) view
    *
    * @param {object}   svg             d3 selection of the SVG element
    * @param {object[]} groups          Group definitions
    * @param {object[]} allNodes        Full tree data
    * @param {number}   svgW            SVG width
    * @param {number}   svgH            SVG height
    * @param {string}   toolMode        "pan" | "select"
    * @param {object}   selectedPaths   Map of path -> selected
    * @param {function} onToggleSelect  Called with path when selection toggled
    * @param {Element}  svgEl           Native SVG element
    * @param {Element}  parent          Parent element
*/
function renderForest(svg, groups, allNodes, svgW, svgH, toolMode, selectedPaths, onToggleSelect, svgEl, parent) {
    ...
    for (var gi = 0; gi < groups.length; gi++) {
        ...
        groupG.selectAll(".forest-link")
            ...
            .attr("stroke", function (d) { return d.target.data._real ? group.color : null; })
            .attr("stroke-opacity", function (d) { return d.target.data._real ? 0.45 : null; });
        ...
        var circles = groupG.selectAll(".forest-node")
            ...
            .attr("r", function (d) { return d.data.type === "dir" ? 5 : 3; })
            .attr("fill", function (d) { return d.data._real ? group.color : null; })
            .attr("fill-opacity", function (d) { return d.data._real ? 0.7 : null; })
            .attr("stroke", function (d) { return d.data._real ? group.color : null; })
            .attr("stroke-width", function (d) { return d.data._real ? 0.5 : null; })
            .attr("stroke-opacity", function (d) { return d.data._real ? 0.3 : null; });

...
function initStableLayout(nodes) {
    if (_initialized || !nodes || !nodes.length) return
    ...
    function computeWeight(n) {
        ...
        if (n.children && n.children.length) {
            ...
            for (var i = 0; i < n.children.length; i++) {
                if (n.children[i].type === "dir") {
                    childDirs.push(n.children[i])
                } else {
                    childFiles.push(n.children[i])
                }
            }
...
function placeChildrenStable(wNodes, startAngle, endAngle, depth, dStep) {
    if (!wNodes || !wNodes.length) return
    ...
    if (r > 0 && adjusted.length > 1) {
        for (var pass = 0; pass < 5; pass++) {
            ...
            for (var i = 1; i < adjusted.length; i++) {
                var cr = adjusted[i].node.data.type === "dir" ? 12 : 7
                var pr = adjusted[i - 1].node.data.type === "dir" ? 12 : 7
...
function renderVisualTree(selector, nodes, expandedPaths, onToggle, toolMode, selectedPaths, groups, onToggleSelect, viewMode) {
    ...
    if (viewMode === "2" && groups && groups.length) {
...
function buildGroupTree(paths, typeLookup) {
    ...
    var root = { name: "", path: "", type: "dir", children: [], _real: false }
    ...
    paths.forEach(function(path) {
        ...
        parts.forEach(function(part, i) {
            ...
            for (var ci = 0; ci < current.children.length; ci++) {
                if (current.children[ci].name === part) {
                    existing = current.children[ci]
                    break
                }
            }
...
function renderForest(svg, groups, allNodes, svgW, svgH, toolMode, selectedPaths, onToggleSelect, svgEl, parent) {
    ...
    for (var gi = 0; gi < groups.length; gi++) {
        ...
        if (!rootData.children || !rootData.children.length) continue
